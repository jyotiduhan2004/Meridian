...
function buildPrompt(body: string, inputs: RunInputs, evidence: string): string {
  ...
  return [
    ...
    "LIMITED EVIDENCE — you were given only a SAMPLE of the project (a handful of files plus, at most, one server-rendered HTML page and/or a single screenshot), NOT the whole codebase. Anything you were not shown is UNKNOWN, not absent.",
    "NEVER claim something is missing, not configured, not implemented, absent, unprotected, or exposed merely because it isn't in the files you were given. A middleware, route, header, config, script, test, sitemap, or robots file can live in a file you didn't receive (e.g. a server entry point). If you can't SEE it, say \"could not verify whether X is present\" and do NOT raise it as a problem.",
    "SEVERITY DISCIPLINE — reserve critical/high ONLY for an issue you can DIRECTLY SEE in the evidence with a concrete impact path. Anything speculative, \"potential\", \"possible\", or based on assumed-absence is at most low and must be worded as could-not-verify. Never raise critical or high on a guess.",
    "Cite a file:line only when that exact line is visible in the provided file content; otherwise cite the file plus a quoted snippet — do not invent or estimate line numbers.",
    "",
    "=== SKILL ===",
    body.slice(0, 6000),
    "",
